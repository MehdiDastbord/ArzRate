            ...
            visible.chunked(count).forEach { rowItems ->
                Row(
                    ...
                    rowItems.forEach { item ->
                        WidgetRateCell(item, data.language, count)
                    }
                    repeat(count - rowItems.size) { Spacer(GlanceModifier.defaultWeight()) }
                }
            }
            if (remaining > 0) {
                Text(
...
@androidx.compose.runtime.Composable
private fun WidgetRateCell(item: RateItem, language: Language, columns: Int) {
...
private fun formatWidgetNumber(raw: String, language: Language): String {
...
class RatesWidgetReceiver : GlanceAppWidgetReceiver() {
    ...
    companion object {
        ...
        fun requestImmediateUpdate(context: Context) {
            ...
            WorkManager.getInstance(context).enqueueUniqueWork(WIDGET_INITIAL_WORK_NAME, ExistingWorkPolicy.REPLACE, request)
        }

        fun scheduleBackgroundUpdates(context: Context) {
            val request = PeriodicWorkRequestBuilder<RatesWidgetUpdateWorker>(15, TimeUnit.MINUTES).build()
            WorkManager.getInstance(context).enqueueUniquePeriodicWork(WIDGET_WORK_NAME, ExistingPeriodicWorkPolicy.UPDATE, request)
        }
    }
}
